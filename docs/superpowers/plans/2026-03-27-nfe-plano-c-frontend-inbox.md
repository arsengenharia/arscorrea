...
**Goal:** Build the NF-e inbox page with real-time updates, review/approval dialog, manual XML upload, manual NF-e entry form, and a badge counter in the sidebar showing pending NF-e count.

**Architecture:** One main page (`/financeiro/nfe`) with 4 tabs: Pendentes (realtime inbox), Historico, Upload Manual (drag-and-drop XML/PDF), and Inserção Manual (form for NF-e received via WhatsApp/paper). Badge in TopNavigation shows count of `aguardando_revisao` items.
...
## Task 6: Manual NF-e Entry Form (Inserção Manual)

**Files:**
- Create: `src/components/financeiro/NfeManualEntryForm.tsx`

- [ ] **Step 1: Create the manual entry form**

This form is for NF-e received via WhatsApp, paper, or any other non-digital channel. It creates a `project_financial_entries` record directly (bypassing the nfe_inbox pipeline) with standardized fields.

```tsx
// src/components/financeiro/NfeManualEntryForm.tsx
import { useState } from "react";
...
- [ ] **Step 2: Add "Inserção Manual" tab to NfeInbox page**

In `src/pages/financeiro/NfeInbox.tsx`, add import:
```typescript
import { NfeManualEntryForm } from "@/components/financeiro/NfeManualEntryForm";
```

Add a 4th TabsTrigger and TabsContent:
```tsx
...
```bash
git add src/components/financeiro/NfeManualEntryForm.tsx src/pages/financeiro/NfeInbox.tsx
git commit -m "feat: add manual NF-e entry form for WhatsApp/paper invoices"
```
...
| 4 | NfeInbox full page (4 tabs: Pendentes, Histórico, Upload, Inserção Manual) | Medium |
| 5 | Badge in TopNavigation | Trivial |
| 6 | Manual NF-e entry form (for WhatsApp/paper NF-e) | Medium |

**Total: 6 tasks, ~18 steps.**

### 3 Input Methods Summary

| Método | Como entra | Pipeline |
|---|---|---|
| **Email automático** | IMAP agent busca a cada 5 min | nfe_inbox → parse → IA → revisão → lançamento |
| **Upload manual** | Drag-and-drop XML/PDF | nfe_inbox → parse → IA → revisão → lançamento |
| **Inserção manual** | Formulário padronizado | Direto → project_financial_entries (pula nfe_inbox) |

After all 3 plans: the complete NF-e module is operational — automatic email fetching (IMAP), XML parsing, AI classification, manual upload, manual form entry, review/approval UI, automatic financial entry creation, real-time notifications.
